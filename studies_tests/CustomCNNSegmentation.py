...
class ResidualBlockWithoutBN(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.skip = nn.Sequential()
        if in_channels != out_channels or stride != 1:
            self.skip = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=True)
...
class SimpleResNetSegmentationWithoutBN(nn.Module):
    def __init__(self, num_classes):
        super(SimpleResNetSegmentationWithoutBN, self).__init__()
        
        # Encoder
        self.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1, bias=True)
        self.relu = nn.ReLU()
        self.resblock1 = ResidualBlockWithoutBN(64, 128, stride=2)  # Adjusted stride
        self.resblock2 = ResidualBlockWithoutBN(128, 256, stride=2)  # Adjusted stride
        self.dropout = nn.Dropout(0.5)
